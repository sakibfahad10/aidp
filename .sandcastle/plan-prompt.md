...
For each unblocked issue, also determine its **assignee**: the single label formatted as `<email>:<username>` (the label containing both an `@` and a `:`, e.g. `saziaalam14@gmail.com:saziaalam14`). Emit that label verbatim as the `assignee` field. If the issue has no such label, emit `"assignee": null`.
...
<plan>
{"issues": [{"id": "42", "title": "Fix auth bug", "branch": "sandcastle/issue-42", "assignee": "dev@example.com:devuser"}]}
</plan>
